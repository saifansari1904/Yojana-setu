  ...
  return (
    ...
    >
      ...
      >
        <div className="report-no-print mb-3 flex items-center justify-between gap-3">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="inline-flex min-h-[44px] items-center gap-2 rounded-xl bg-[#175741] px-4 text-sm font-semibold text-white transition-colors hover:bg-[#15803D] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"
            >
        ...
        >
          <header className="mb-5 border-b border-[#E4E8E4] pb-4">
            ...
            <p className="mt-1 text-xs text-[#516A5F]">
          ...
          <section className="mb-5 grid gap-x-6 gap-y-1 sm:grid-cols-2">
            ...
            {report.profileLines.map((line) => (
              <div key={line.label} className="flex justify-between gap-3 text-sm">
                <span className="text-[#516A5F]">{line.label}</span>
          ...
          <section className="mb-5">
            ...
            {report.schemes.length === 0 ? (
              <p className="text-sm text-[#516A5F]">
                {t('report.noSchemes')}
              </p>
            ) : (
              <ul className="space-y-2">
                {report.schemes.map((scheme, index) => (
                  <li
                    ...
                    className="rounded-lg border border-[#E4E8E4] p-3 text-sm"
                  >
                    ...
                    {scheme.sourceUrl && (
                      <p className="mt-0.5 break-all text-xs text-[#1E6A50]">{scheme.sourceUrl}</p>
                    )}
                  </li>
                ))}
              </ul>
            )}
            <p className="mt-2 text-xs text-[#516A5F]">
            ...
            <ul className="space-y-1 text-sm">
              {report.readiness.checks.map((check) => (
                <li key={check.label} className="flex justify-between gap-3">
                  <span className="text-[#516A5F]">{check.label}</span>
              ...
              <ul className="space-y-1 text-sm">
                {report.checklist.map((item, index) => (
                  <li key={`${item.label}-${index}`} className="flex justify-between gap-3">
                    ...
                    <span className="text-[#516A5F]">{item.stateLabel}</span>
                  </li>
                ))}
              </ul>
            </section>
              ...
              <ul className="space-y-1 text-sm">
                {report.trackedLines.map((line) => (
                  <li key={line.label} className="flex justify-between gap-3">
                    ...
                    <span className="capitalize text-[#516A5F]">{line.value}</span>
                  </li>
                ))}
              </ul>
            </section>
          ...
          <footer className="border-t border-[#E4E8E4] pt-3">
            <ul className="space-y-1 text-xs text-[#516A5F]">
