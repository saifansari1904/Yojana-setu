    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[var(--overlay)] backdrop-blur-xs">
        <motion.div
          ...
          className="w-full max-w-lg bg-white dark:bg-[var(--bg-card)] rounded-2xl shadow-xl border border-[#E5E9E7] dark:border-[var(--border-subtle)] overflow-hidden"
        >
          {/* Modal Header */}
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div>
              <label className="block text-xs font-semibold text-[#1F2421] dark:text-[var(--text-main)] mb-1">
              ...
              <input
                ...
                className="w-full px-3.5 py-2 text-xs rounded-xl border border-[#E5E9E7] dark:border-[var(--border-subtle)] bg-white dark:bg-[var(--bg-raised)] text-[#1F2421] dark:text-[var(--text-main)] focus:outline-hidden focus:ring-2 focus:ring-[#1E6A50]"
              />
            </div>
            ...
            <div className="pt-2">
              <label className="flex items-start gap-2.5 cursor-pointer">
                ...
                />
                <span className="text-xs text-[#1F2421] dark:text-[var(--text-main)] leading-snug">
            ...
            {/* Actions */}
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-[#E5E9E7] dark:border-[var(--border-subtle)]">
              <button
                type="button"
                onClick={onClose}
                className="px-4 py-2 text-xs font-semibold text-[#5A6561] dark:text-[var(--text-secondary)] hover:bg-[#F4F7F5] dark:hover:bg-[var(--bg-raised)] rounded-xl transition-colors"
              >
                {t('workspace.cancelBtn')}
              </button>
              <button
                type="submit"
                className="px-4 py-2 text-xs font-semibold text-white bg-[#1E6A50] hover:bg-[#0D5B41] dark:bg-[var(--accent-green)] dark:text-[#0E1311] dark:hover:bg-[#22C55E] rounded-xl transition-colors shadow-sm"
              >
