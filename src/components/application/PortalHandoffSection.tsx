...
export const PortalHandoffSection: React.FC<PortalHandoffSectionProps> = ({
  ...
  return (
    <div id="portal-handoff-section" className="space-y-6">
      ...
      <div className="p-6 rounded-2xl border border-[#E5E9E7] dark:border-[var(--border-subtle)] bg-white dark:bg-[var(--bg-card)] space-y-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          ...
          {scheme.officialPortalUrl ? (
            ...
            <span className="text-xs text-[#5A6561] dark:text-[var(--text-secondary)] italic">
              Official portal link not declared in dataset. Please apply via local District Industries Centre (DIC).
            </span>
          )}
        </div>

        <div className="pt-4 border-t border-[#E5E9E7] dark:border-[var(--border-subtle)] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-semibold border border-[#1E6A50] dark:border-[var(--accent-green)] text-[#1E6A50] dark:text-[var(--accent-green)] bg-[#1E6A50]/5 hover:bg-[#1E6A50]/10 transition-colors"
          >
