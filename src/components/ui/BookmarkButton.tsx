  ...
  return (
    <motion.button
      ...
      className={`yj-tap yj-focus-ring relative inline-flex items-center justify-center font-bold rounded-[var(--yj-radius-md)] transition-all cursor-pointer select-none border ${
        isSaved
          ? 'bg-[#D9E8DF] dark:bg-[#1A382D] text-[#14453D] dark:text-[var(--accent-green)] border-[#1E6A50] dark:border-[#22C55E]'
          : 'bg-white dark:bg-[var(--bg-raised)] hover:bg-[#F3F4F3] dark:hover:bg-[#26352E] text-[#516A5F] dark:text-[var(--text-secondary)] border-[#E4E8E4] dark:border-[var(--border-strong)] hover:border-[#B2CDBF] dark:hover:border-[#3E5C4E]'
    ...
    >
      ...
      {justToggled && isSaved && !shouldReduceMotion && (
        <motion.span
          ...
          className="pointer-events-none absolute inset-0 rounded-[var(--yj-radius-md)] border border-[#1E6A50] dark:border-[var(--accent-green)]"
        />
      )}
      ...
      >
        <Heart
          className={`w-3.5 h-3.5 transition-colors ${
            isSaved
              ? 'fill-[#1E6A50] dark:fill-[#4ADE80] text-[#1E6A50] dark:text-[var(--accent-green)]'
              : 'text-current'
          }`}
        />
      </motion.div>
