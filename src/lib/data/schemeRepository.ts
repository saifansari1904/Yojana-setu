 ...
 */
export function getSchemesByBusinessType(businessType: BusinessType): Scheme[] {
  return SCHEMES_DATABASE.filter((s) => s.targetBusinessTypes.includes(businessType));
}

/**
 * Retrieves schemes targeting a designated social category.
 */
export function getSchemesByCategory(category: SocialCategory): Scheme[] {
  return SCHEMES_DATABASE.filter(
    (s) => s.targetCategories.includes(category) || s.targetCategories.includes('General')
  );
}
 ...
 */
export function getSchemesBySector(sector: Sector): Scheme[] {
  return SCHEMES_DATABASE.filter((s) => {
    if (s.intelligence?.targeting?.sector) {
      return s.intelligence.targeting.sector.includes(sector);
    }
    return true;
  });
}
 ...
 */
export function getSchemesCount(): number {
  return SCHEMES_DATABASE.length;
}
 ...
 */
export function getSchemesByFreshness(
  status: FreshnessStatus,
  schemesPool: Scheme[] = SCHEMES_DATABASE
): Scheme[] {
  return schemesPool.filter((s) => {
    const profile = s.trustProfile || deriveSchemeTrustProfile(s);
    return profile.freshness.status === status;
  });
}
 ...
 */
export function getSchemesByConfidence(
  level: ConfidenceLevel,
  schemesPool: Scheme[] = SCHEMES_DATABASE
): Scheme[] {
  return schemesPool.filter((s) => {
    const profile = s.trustProfile || deriveSchemeTrustProfile(s);
    return profile.confidence === level;
  });
}
